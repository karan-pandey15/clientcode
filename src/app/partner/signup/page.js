...
const PartnerSignUp = () => {
  ...
  const [userType, setUserType] = useState("Partner's Team Leader");
  ...
  const handleSubmit = (event) => {
    if (
      (userType === "Partner's Team Leader" &&
        secretKey !== "PartnerTlKey123") ||
      (userType === "Partner" && secretKey !== "PartnerKey123")
    ) {
      ...
      axios
        .post("https://api.addrupee.com/api/p_emp_register", registerData)
        .then((res) => {
          if (res.status !== 200 || !res.data) {
            alert("Error while registering. Please try again.");
          } else if (res.data.Status === "Success") {
            alert(res.data.Message);
            router.push("/partner/otpverification");
          } else {
                ...
                <div className="p-4">
                  <div>
                    ...
                    <input
                      type="radio"
                      name="userType"
                      value="Partner's Team Leader"
                      onChange={handleRadioChange}
                      style={{ marginLeft: "6px", cursor: "pointer" }}
                      checked={userType === "Partner's Team Leader"}
                    />
                    Team Manager
                    <input
                      type="radio"
